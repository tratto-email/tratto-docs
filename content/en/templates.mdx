...
Move a template from `draft` to `published` so it can be used for sending.
...
This creates a new version (3) with the content of version 1 in `draft` status.
...
- **Send with a template?** See [Send Email](/docs/en/send-email)
- **Use templates in campaigns?** Go to [Campaigns](/docs/en/campaigns)
- **Use templates in flows?** See [Flows](/docs/en/flows)
