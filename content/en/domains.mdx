...
| SPF | TXT | `hello.yourdomain.com` | `v=spf1 include:amazonses.com ~all` | Recommended |
...
This authorizes Amazon SES to send for the hostname. A hostname can have only one SPF record: if it already has one, add `include:amazonses.com` to it instead of creating a second.
