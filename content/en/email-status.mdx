...
| `failed` | Email delivery failed (invalid domain, mailbox full, an unpublished template, etc.). The reason is recorded on the email and shown in the dashboard; the API returns the status, not the reason |
