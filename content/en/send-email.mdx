...
Depending on your plan, Tratto may append a footer to the HTML version — the "Sent using Tratto" badge on the free plan (optional on paid plans). See [Email Footer & Branding](/en/docs/branding).
...
[emailmd format](/en/docs/emailmd-templates):
...
`subject` is required on every send. The template's `html` (and, for an
`emailmd` template, its pinned text part) is rendered with the variables you
pass; **the subject is not** — it goes out exactly as written.
...
| `markdown` | string | | [emailmd markdown](/en/docs/emailmd-templates), rendered server-side (mutually exclusive with `html`, max 100,000 chars) |
...
**Cause:** You have exceeded the rate limit of 500 requests per minute.
