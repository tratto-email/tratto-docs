...
`unsubscribed` isn't part of this endpoint — it's tracked on the [contact](/en/docs/contacts), not on individual emails. Check a contact's `status` instead. A campaign has no `unsubscribed` count either — the nearest thing is `stats.skipped`, the sends cancelled because the contact unsubscribed between dispatch and delivery (see [Campaigns](/en/docs/campaigns)).
