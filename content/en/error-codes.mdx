...
| `QUOTA_EXCEEDED` | 429 | A plan limit was reached: the monthly email quota, the domain limit, or the daily test-mode cap | Upgrade the plan or wait for the reset |
...
- **`QUOTA_EXCEEDED`**: you have hit a plan limit, such as the monthly email
  allowance. Retrying will not help until the limit resets or the plan is
  upgraded.
