...
A `templateId` that does not exist is a `NOT_FOUND` (404) instead. Test sends
are exempt, and `POST /v1/emails` is not refused at all: it accepts the
request and the email fails at delivery. See
