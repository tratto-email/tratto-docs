...
A template has to be `published` before it can be used to send, so
`VALIDATION_ERROR` (422) also covers `POST /v1/campaigns`, a
`PATCH /v1/campaigns/:id` that swaps in a template, and
