...
yet. On the current 1.0.0 release these examples fail — until then, call the
[REST API directly](/en/docs/send-email#send-with-markdown).
</Callout>
...
(creating `format: 'emailmd'` templates), with `format`, `source` and
`renderWarnings` in the template responses.
