...
app.post('/webhooks/tratto', (req, res) => {
  const signature = req.header('x-tratto-signature') ?? '';

  if (!verify((req as any).rawBody, signature, process.env.TRATTO_WEBHOOK_SECRET!)) {
...
export async function POST(request: Request) {
  ...
  const signature = request.headers.get('x-tratto-signature') ?? '';

  if (!(await verify(rawBody, signature, process.env.TRATTO_WEBHOOK_SECRET!))) {
