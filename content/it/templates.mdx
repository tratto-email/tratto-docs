...
Pubblicare è un passaggio, non un'etichetta: il controllo è
`status !== "published"`, quindi qualsiasi valore diverso da `published` viene
rifiutato.
...
l'email finisce con `status: "failed"` e non parte nulla. Il motivo registrato
è `Template 'tmpl_abc123' is not published and cannot be used to send.`
La dashboard lo mostra, `GET /v1/emails/:id` restituisce lo status ma non il
motivo. Lo stesso vale se un template viene de-pubblicato dopo che un'email è
già stata messa in coda con quel template.
