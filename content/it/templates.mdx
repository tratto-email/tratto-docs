...
flusso end-to-end: [Template Markdown (emailmd)](/it/docs/emailmd-templates).
...
<Callout type="warn">
**Le variabili non vengono sostituite nell'oggetto.** La sostituzione gira solo
sui corpi HTML e testo. Un oggetto `"Benvenuto, {{firstName}}!"` viene
recapitato esattamente così, graffe comprese. Componi l'oggetto tu prima di
inviare.
</Callout>

Un template `html` non ha una parte testuale. Un template `emailmd` fissa al
...
Per i template `emailmd`, aggiorna invece il campo `markdown` — ogni modifica
...
Su un template `emailmd` la risposta porta anche `source`: il markdown che ha
...
La sostituzione si applica ai corpi HTML e testo. **Non all'oggetto** — vedi
sopra.
...
Vale per l'oggetto e per il corpo, in HTML e in emailmd, sia sugli invii
transazionali che sulle campagne. Anche `null` sostituisce vuoto: non
comparirà mai la stringa `null` o `undefined` in un'email.

Non è un invito a passare meno variabili. Una frase con un buco resta una
frase sbagliata, e all'invio non te lo segnala nessuno — quindi passa sempre
tutti i placeholder che il template usa. Quello che la dashboard controlla è
un caso diverso e più subdolo: se scrivi `{{firstname}}` dove il campo si
chiama `firstName`, il refuso viene segnalato in bozza (avviso
`unknown_variable`), perché una variabile che non esiste non verrà mai
sostituita da niente.
