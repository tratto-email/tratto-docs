...
Una variabile mancante **non** fa fallire l'invio. Il placeholder resta nel
messaggio esattamente com'è scritto, e il destinatario legge nella sua inbox
`Ciao {{firstName}}, benvenuto a {{companyName}}!`.

```html
<p>Ciao {{firstName}}, benvenuto a {{companyName}}!</p>
```

Nessun controllo lato server te lo segnala, quindi passa a ogni invio tutti i
placeholder usati dal template: una stringa vuota è un default migliore di un
`{{firstName}}` visibile.
