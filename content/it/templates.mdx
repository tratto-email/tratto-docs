...
<Callout type="info">
**I test-send sono esenti.** `POST /v1/templates/:id/test-send` e
`POST /v1/campaigns/:id/test-send` funzionano anche su un template `draft`:
controllare una bozza in una casella vera è esattamente il punto, ed è l'unico
modo di rivederla prima di pubblicarla.
</Callout>
