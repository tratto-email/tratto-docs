...
pubblicata. Sulla release 1.0.0 attuale questi esempi falliscono — nel
...
`templates.update` (creando template `format: 'emailmd'`), con `format`,
`source` e `renderWarnings` nelle risposte dei template.
...
print(stats['data']['rates']['openRate'])    # 50 → 50%, i tassi sono percentuali
```
