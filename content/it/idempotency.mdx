...
L'idempotenza è supportata sulle operazioni POST critiche:
- `POST /v1/emails` — Invio email
- `POST /v1/campaigns/{id}/send` — Invio campagna
- `POST /v1/flows` — Creazione flow
- `POST /v1/contacts/import` — Importazione massiva
...
Se usi la stessa chiave con un **corpo della richiesta diverso** ricevi un errore 409:
...
**Soluzione:** usa una nuova chiave di idempotenza per una richiesta diversa.
