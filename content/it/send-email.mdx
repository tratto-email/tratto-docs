...
Prima di poter inviare email, hai bisogno di un **dominio di invio verificato**. Vedi [Domini](/docs/it/domains) per la configurazione.
...
**Step 1: Crea un template (vedi [Template](/docs/it/templates))**
...
**Fix:** Completa il [processo di verifica del dominio](/docs/it/domains).

### RATE_LIMIT_EXCEEDED

**Causa:** Hai superato il limite di velocità del tuo tenant (500 richieste/minuto).

**Fix:** Implementa backoff esponenziale e riprova più tardi. Vedi [Rate Limits](/docs/it/rate-limits).

### VALIDATION_ERROR

**Causa:** Richiesta non valida (campo mancante, JSON malformato, ecc.).

**Fix:** Controlla il messaggio di errore e consulta il riferimento [Codici di Errore](/docs/it/error-codes).

## Prossimi Passi

- **Tracciare gli eventi email?** Configura [Webhook](/docs/it/webhooks)
- **Controllare lo stato dell'email?** Vedi [Stato e Ciclo di Vita dell'Email](/docs/it/email-status)
- **Creare template?** Vai a [Template](/docs/it/templates)
- **Gestire domini?** Leggi [Domini](/docs/it/domains)
