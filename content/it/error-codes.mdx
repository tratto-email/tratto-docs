...
description: I codici di errore restituiti dall'API Tratto, e cosa fare per ciascuno.
---
...
| `TEMPLATE_NOT_PUBLISHED` | 422 | Il template indicato nella richiesta è ancora in bozza e non può essere usato per inviare | Pubblicalo (`PATCH /v1/templates/:id` con `status: "published"`), poi ripeti la richiesta |
...
<Callout type="warn">
Una risorsa inesistente non ha un codice tutto suo. Un template inesistente, un
contatto inesistente e una campagna inesistente restituiscono tutti
`NOT_FOUND`: la risorsa è indicata in `message`, non in `code`.
</Callout>
...
Un template deve essere `published` prima di poter essere usato per inviare.
È un codice a sé, non una variante di `VALIDATION_ERROR`: non porta l'array
`details`, perché nessuno schema ha rifiutato il corpo. Lo restituiscono
`POST /v1/campaigns`, una `PATCH /v1/campaigns/:id` che sostituisce il template
e `POST /v1/flows/:id/activate`:
...
All'attivazione di un flow il messaggio indica anche lo step che usa il
template:
`Template 'tmpl_abc123' on step 's0' is not published and cannot be used to send.`

Un `templateId` inesistente è invece un `NOT_FOUND` (404). Il test-send del
template è esente, e `POST /v1/emails` non viene rifiutata affatto: accetta la
richiesta e l'email fallisce in consegna. Vedi
...
- **`QUOTA_EXCEEDED`**: hai raggiunto un limite d'uso, come la quota mensile di
  email. Riprovare non serve finché il limite non si azzera, non elimini un
  dominio o non passi a un piano superiore.
...
[Campagne](/it/docs/campaigns)); riprenderla più tardi raggiunge solo i
  ...
  if (error instanceof TrattoError) {
    switch (error.code) {
      ...
      case 'TEMPLATE_NOT_PUBLISHED':
        // Arriva dalle chiamate su campagne e flussi. Pubblica il template,
        // poi rifai la chiamata: riprovare così com'è non serve.
        break;
      case 'RATE_LIMITED':
        await backoff();
        break;
