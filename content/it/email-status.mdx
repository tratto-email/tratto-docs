...
| `failed` | Consegna email non riuscita (dominio non valido, mailbox piena, un template non pubblicato, ecc.). Il motivo viene registrato sul documento dell'email, ma oggi nessuno lo espone: `GET /v1/emails/:id` restituisce lo status e non il motivo, e la dashboard legge quello stesso endpoint |
...
- **Configurare webhook real-time?** Vedi [Webhook](/it/docs/webhooks)
- **Inviare email?** Vai a [Invia Email](/it/docs/send-email)
- **Comprendi la gestione bounce/complaint?** Leggi [Deliverability](/it/docs/deliverability)
